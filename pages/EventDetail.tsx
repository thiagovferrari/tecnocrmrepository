...
import { SponsorshipStatus, Company, Contact } from '../types';
import { StatusBadge } from '../components/StatusBadge';
import { ArrowLeft, Search, Plus, Download, Edit3, Save, X, UserPlus, Trash2, MapPin, Calendar } from 'lucide-react';

export const EventDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const event = db.getEvents().find(e => e.id === id);
  const [relations, setRelations] = useState(db.getRelations().filter(r => r.event_id === id));
  ...
  }, [relations, companies, search, statusFilter]);
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3 sticky bottom-0 z-10">
          ...
          <button onClick={handleAdd} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all">Vincular Agora</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200">
        ...
        <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex gap-4">
          ...
          <button onClick={handleSave} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg shadow-blue-100 flex items-center justify-center gap-2 transition-all">
